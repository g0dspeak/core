...
class Database extends Backend implements IUserBackend, IProvidesHomeBackend, IProvidesDisplayNameBackend {
	...
	public function getUsers($search = '', $limit = null, $offset = null) {
		...
		if ($allowMedialSearches) {
			$parameter = '%' . $db->escapeLikeParameter($search) . '%';
			$loweredParameter = '%' . $db->escapeLikeParameter(\strtolower($search)) . '%';
		} else {
			$parameter = $db->escapeLikeParameter($search) . '%';
			$loweredParameter = $db->escapeLikeParameter(\strtolower($search)) . '%';
		}
		...
		$qb->select('user_id')
			...
			->where($qb->expr()->eq('backend', $qb->createNamedParameter(self::class)))
			->andWhere($qb->expr()->orX(
				$qb->expr()->iLike('lower_user_id', $qb->createNamedParameter($parameter)),
				$qb->expr()->iLike('user_id', $qb->createNamedParameter($parameter)),
				$qb->expr()->iLike('display_name', $qb->createNamedParameter($parameter)),
				$qb->expr()->iLike('email', $qb->createNamedParameter($parameter)),
